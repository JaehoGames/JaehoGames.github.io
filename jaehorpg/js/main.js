...
function handleAuthError(error) {
    ...
    switch (error.code) {
        ...
        case 'auth/wrong-password': message.textContent = '아이디 혹은 비번이 틀렸어요.'; break;
...
registerButton.addEventListener('click', async () => {
    ...
    // 유효성 검사
    if (!username || !email || !password || !nickname) { message.textContent = '빈칸을 모두 채워주세요.'; return; }
...
loginButton.addEventListener('click', async () => {
    ...
    if (!username || !password) { message.textContent = '아이디랑 비번 입력해주세요.'; return; }
    
    message.textContent = '로그인 시도 중...';
    try {
        ...
        if (querySnapshot.empty) { message.textContent = '없는 아이디입니다.'; return; }
...
const ZOOM_LEVEL = 4.0; // 확대 배율
const SCALED_TILE_SIZE = TILE_SIZE * ZOOM_LEVEL;
...
const scaledTileCache = []; // 확대된 타일 이미지를 미리 저장해두는 배열
...
tileset.onload = () => {
    ...
    for (let i = 0; i < totalTiles; i++) {
        ...
        scaledCanvas.width = SCALED_TILE_SIZE;
        scaledCanvas.height = SCALED_TILE_SIZE;
        const scaledCtx = scaledCanvas.getContext('2d');
        scaledCtx.imageSmoothingEnabled = false;
        scaledCtx.drawImage(tempCanvas, 0, 0, SCALED_TILE_SIZE, SCALED_TILE_SIZE);
...
objectTileset.onload = () => {
    ...
    const tempCtx = tempCanvas.getContext('2d', { willReadFrequently: true }); // 자주 읽을 거라 최적화 옵션 켬
    ...
    for (let i = 0; i < totalTiles; i++) {
        ...
        // 2. 렌더링용 캐시 생성
        const SCALED_OBJECT_TILE_SIZE = OBJECT_TILE_SIZE * ZOOM_LEVEL;
        const scaledCanvas = document.createElement('canvas');
        scaledCanvas.width = SCALED_OBJECT_TILE_SIZE;
        scaledCanvas.height = SCALED_OBJECT_TILE_SIZE;
        const scaledCtx = scaledCanvas.getContext('2d');
        scaledCtx.imageSmoothingEnabled = false;
        scaledCtx.drawImage(tempCanvas, 0, 0, SCALED_OBJECT_TILE_SIZE, SCALED_OBJECT_TILE_SIZE);
...
function drawTile(tileValue, worldX, worldY, cameraX, cameraY) {
    ...
    if (tilesetLoaded && tileIndex >= 0 && scaledTileCache[tileIndex]) {
        const drawX = Math.floor((worldX - cameraX) * ZOOM_LEVEL);
        const drawY = Math.floor((worldY - cameraY) * ZOOM_LEVEL);
        ctx.drawImage(scaledTileCache[tileIndex], drawX, drawY);
    } else {
        // 이미지 로딩 전이면 회색 박스로 대체
        const drawX = Math.floor((worldX - cameraX) * ZOOM_LEVEL);
        const drawY = Math.floor((worldY - cameraY) * ZOOM_LEVEL);
        ctx.fillStyle = '#4a4a4a';
        ctx.fillRect(drawX, drawY, SCALED_TILE_SIZE, SCALED_TILE_SIZE);
    }
}
...
function drawObjectTile(tileValue, worldX, worldY, cameraX, cameraY) {
    ...
    if (objectTilesetLoaded && tileIndex > 0 && scaledObjectTileCache[tileIndex]) {
        const objectScaledHeight = OBJECT_TILE_SIZE * ZOOM_LEVEL;
        const tileScaledHeight = TILE_SIZE * ZOOM_LEVEL;

        const drawX = Math.floor((worldX - cameraX) * ZOOM_LEVEL);
        // 오브젝트는 타일보다 크니까 바닥 기준점을 맞춰서 위로 그림
        const drawY = Math.floor((worldY - cameraY) * ZOOM_LEVEL) - (objectScaledHeight - tileScaledHeight);
        
        ctx.drawImage(scaledObjectTileCache[tileIndex], drawX, drawY);
    } else {
        // 로딩 안됐을 때 대체 박스
        // ... (생략 가능하나 디버깅 위해 남겨둠)
    }
...
function drawMap(cameraX, cameraY) {
    ...
    const viewRight = Math.ceil((cameraX + canvas.width / ZOOM_LEVEL) / TILE_SIZE);
    const viewBottom = Math.ceil((cameraY + canvas.height / ZOOM_LEVEL) / TILE_SIZE);
...
function drawPlayer(cameraX, cameraY) {
    const scaledCollisionWidth = playerState.width * ZOOM_LEVEL;
    const scaledCollisionHeight = playerState.height * ZOOM_LEVEL;
    const drawX = Math.floor((playerState.x - cameraX) * ZOOM_LEVEL);
    const drawY = Math.floor((playerState.y - cameraY) * ZOOM_LEVEL);
    ...
    if (useSprite) {
        ...
        if (animation) {
            ...
            if (frame) {
                ...
                const scaledRenderWidth = renderWidth * ZOOM_LEVEL;
                const scaledRenderHeight = renderHeight * ZOOM_LEVEL;
...
async function drawNpc(npc, cameraX, cameraY) {
    ...
    const scaledCollisionWidth = npc.width * ZOOM_LEVEL;
    const scaledCollisionHeight = npc.height * ZOOM_LEVEL;
    const drawX = Math.floor((npc.x - cameraX) * ZOOM_LEVEL);
    const drawY = Math.floor((npc.y - cameraY) * ZOOM_LEVEL);
...
function gameLoop(currentTime) {
    ...
    const cameraX = playerState.x + playerState.width / 2 - (canvas.width / ZOOM_LEVEL / 2);
    const cameraY = playerState.y + playerState.height / 2 - (canvas.height / ZOOM_LEVEL / 2);
