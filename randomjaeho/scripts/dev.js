...
function initDevPanel() {
    ...
    document.getElementById('forceStratosphereEventButton')?.addEventListener('click', toggleStratosphereEvent);
    document.getElementById('forceDanYuEventButton')?.addEventListener('click', toggleDanYuEvent);
    document.getElementById('forceClubEventButton')?.addEventListener('click', toggleClubEvent);
...
async function inspectUser(uid) {
    ...
    if (uid === 'all') {
        ...
        const danyuProbInput = document.getElementById('prob-mutation-danyu');
        const skyBlueProbInput = document.getElementById('prob-mutation-sky_blue');
        ...
        if (danyuProbInput) danyuProbInput.value = MUTATION_CONFIG.DANYU.probability;
        if (skyBlueProbInput) skyBlueProbInput.value = MUTATION_CONFIG.SKY_BLUE.probability;
    ...
    try {
        ...
        const flameProbInput = document.getElementById('prob-mutation-flame');
        const danyuProbInput = document.getElementById('prob-mutation-danyu');
        const neonProbInput = document.getElementById('prob-mutation-neon');
        const skyBlueProbInput = document.getElementById('prob-mutation-sky_blue');
        ...
        if (danyuProbInput) {
            danyuProbInput.value = customMutationProbs.danyu ?? MUTATION_CONFIG.DANYU.probability;
        }
        if (neonProbInput) {
            neonProbInput.value = customMutationProbs.neon ?? MUTATION_CONFIG.NEON.probability;
        }
        if (skyBlueProbInput) {
            skyBlueProbInput.value = customMutationProbs.sky_blue ?? MUTATION_CONFIG.SKY_BLUE.probability;
        }
...
/**
 * 관리자 알림을 모든 사용자에게 실시간으로 전송합니다.
 * @param {string} title - 알림 제목
 * @param {string} message - 알림 메시지
 * @param {string} icon - 표시할 아이콘 (이모지)
 * @param {'info' | 'success' | 'warning' | 'error'} type - 알림 종류
 */
async function broadcastAdminNotification(title, message, icon, type = 'info') {
    try {
    ...
    // 알림을 보낸 관리자에게도 즉시 표시됩니다.
    showAdminNotification(title, message, icon, type);
}

/**
 * 오래된 관리자 알림을 삭제하는 클라우드 함수를 호출합니다. (참고용)
 * 실제로는 Firebase Cloud Functions에 배포해야 합니다.
 */
async function cleanupOldAdminNotifications() {
    // 예시: 2분 이상 된 알림 삭제
    const twoMinutesAgo = firebase.firestore.Timestamp.fromMillis(Date.now() - 2 * 60 * 1000);
    const oldNotifs = await db.collection('admin_notifications').where('timestamp', '<', twoMinutesAgo).get();
...
async function toggleRealEvent() {
    ...
    try {
        ...
        broadcastAdminNotification('⏰ 실제 이벤트', `이벤트가 성공적으로 ${newStatus ? '시작' : '종료'}되었습니다.`, '⏰');
    } catch (error) {
        console.error("실제 이벤트 상태 변경 실패:", error);
        broadcastAdminNotification('오류', `이벤트 상태 변경 실패: ${error.message}`, '❌', 'error');
    }
}
/**
 * 개발자 패널에서 클럽 파티 이벤트를 강제로 시작/종료합니다.
 */
async function toggleClubEvent() {
    const messageEl = document.getElementById('devPanelMessage');
    const eventStateRef = db.collection('globals').doc('eventState');

    try {
        const doc = await eventStateRef.get();
        const currentStatus = doc.exists ? doc.data().clubEventActive : false;
        const newStatus = !currentStatus;

        await eventStateRef.set({ 
            clubEventActive: newStatus
        }, { merge: true });
 
        broadcastAdminNotification('🎉 클럽 파티', `이벤트가 성공적으로 ${newStatus ? '시작' : '종료'}되었습니다.`, '🎉');
    } catch (error) {
        console.error("클럽 파티 이벤트 상태 변경 실패:", error);
        broadcastAdminNotification('오류', `이벤트 상태 변경 실패: ${error.message}`, '❌', 'error');
    }
}

/**
 * 개발자 패널에서 최단유 이벤트를 강제로 시작/종료합니다.
 */
async function toggleDanYuEvent() {
    const messageEl = document.getElementById('devPanelMessage');
    const eventStateRef = db.collection('globals').doc('eventState');

    try {
        const doc = await eventStateRef.get();
        const currentStatus = doc.exists ? doc.data().danYuEventForced : false;
        const newStatus = !currentStatus;

        await eventStateRef.set({ 
            danYuEventForced: newStatus
        }, { merge: true });
 
        broadcastAdminNotification('💧 최단유 이벤트', `이벤트가 성공적으로 ${newStatus ? '시작' : '종료'}되었습니다.`, '💧');
    } catch (error) {
        console.error("최단유 이벤트 상태 변경 실패:", error);
        broadcastAdminNotification('오류', `이벤트 상태 변경 실패: ${error.message}`, '❌', 'error');
    }
}
...
async function updateUserProbabilities() {
    ...
    try {
        ...
        if (uid === 'all') {
            ...
            await batch.commit();
            broadcastAdminNotification('📊 확률 변경', `모든 유저(${usersSnapshot.size}명)의 등급 확률이 업데이트되었습니다.`, '📊');
        } else if (uid) { // 특정 유저에게 적용
            await db.collection('users').doc(uid).update({ 'stats.customProbabilities': newProbs });
            broadcastAdminNotification('📊 확률 변경', `선택된 유저의 등급 확률이 업데이트되었습니다.`, '📊');
...
async function updateUserMutationProbabilities() {
    ...
    try {
        ...
        const danyuProb = parseFloat(danyuProbInput.value);
        const neonProb = parseFloat(neonProbInput.value);
        const skyBlueProb = parseFloat(skyBlueProbInput.value);

        if (isNaN(goldProb) || isNaN(rainbowProb) || isNaN(flameProb) || isNaN(danyuProb) || isNaN(neonProb) || isNaN(skyBlueProb) || goldProb < 0 || rainbowProb < 0 || flameProb < 0 || danyuProb < 0 || neonProb < 0 || skyBlueProb < 0) {
            throw new Error('유효하지 않은 확률 값입니다. 0 이상의 숫자를 입력하세요.');
        }

        const newMutationProbs = { gold: goldProb, rainbow: rainbowProb, flame: flameProb, danyu: danyuProb, neon: neonProb, sky_blue: skyBlueProb };

        if (uid === 'all') {
            // 모든 유저에게 변이 확률 적용
            if (!confirm(`정말로 모든 유저의 변이 확률을 골드 ${goldProb}%, 레인보우 ${rainbowProb}%, 화염 ${flameProb}%, 최단유 ${danyuProb}%, 네온 ${neonProb}%, 하늘빛 ${skyBlueProb}%로 설정하시겠습니까?`)) {
                return;
            }
            ...
            await batch.commit();
            broadcastAdminNotification('✨ 변이 확률 변경', `모든 유저(${usersSnapshot.size}명)의 변이 확률이 업데이트되었습니다.`, '✨');

        } else if (uid) { // 특정 유저에게 적용
            // 특정 유저에게 적용
            await db.collection('users').doc(uid).update({ 'stats.customMutationProbabilities': newMutationProbs });
            broadcastAdminNotification('✨ 변이 확률 변경', `선택된 유저의 변이 확률이 업데이트되었습니다.`, '✨');
...
function resetMutationProbabilitiesToDefault() {
    ...
    try {
        ...
        document.getElementById('prob-mutation-danyu').value = MUTATION_CONFIG.DANYU.probability;
        document.getElementById('prob-mutation-neon').value = MUTATION_CONFIG.NEON.probability;
        document.getElementById('prob-mutation-sky_blue').value = MUTATION_CONFIG.SKY_BLUE.probability;
...
async function toggleFlameEvent() {
    ...
    try {
        ...
        broadcastAdminNotification('🔥 화염 이벤트', `이벤트가 성공적으로 ${newStatus ? '시작' : '종료'}되었습니다.`, '🔥');
    } catch (error) {
...
/**
 * 개발자 패널에서 성층권 이벤트를 강제로 시작/종료합니다.
 */
async function toggleStratosphereEvent() {
    const messageEl = document.getElementById('devPanelMessage');
    const eventStateRef = db.collection('globals').doc('eventState');

    try {
        const doc = await eventStateRef.get();
        const currentStatus = doc.exists ? doc.data().stratosphereEventForced : false;
        const newStatus = !currentStatus;
        ...
        if (newStatus) {
            ...
            const endTime = new Date(Date.now() + 10 * 60 * 1000);
            await eventStateRef.set({ 
                stratosphereEventForced: true,
                stratosphereEventForcedEndTime: firebase.firestore.Timestamp.fromDate(endTime)
            }, { merge: true });
        } else {
            // 이벤트 종료: 관련 필드 초기화
            await eventStateRef.set({ stratosphereEventForced: false, stratosphereEventForcedEndTime: null }, { merge: true });
        }

        broadcastAdminNotification('🌌 성층권 이벤트', `이벤트가 성공적으로 ${newStatus ? '시작' : '종료'}되었습니다.`, '🌌');
    } catch (error) {
        console.error("성층권 이벤트 상태 변경 실패:", error);
        broadcastAdminNotification('오류', `이벤트 상태 변경 실패: ${error.message}`, '❌', 'error');
    }
}
