...
function isHalloweenPeriod() {
    // 1주차 업데이트에서는 테스트를 위해 항상 true를 반환합니다.
    return true;
}
    ...
    setTimeout(() => {
        try {
            ...
            if (isHalloweenPeriod()) {
                const candyGained = Math.floor(Math.random() * 3) + 1; // 1~3개 획득
                stats.ghostCandy += candyGained;
                updateGhostCandyDisplay(); // 유령 사탕 개수 UI 업데이트
                // 유령 사탕 획득 UI 업데이트 (알림)
                if (typeof showNotification === 'function') {
                    setTimeout(() => { // 뽑기 결과와 겹치지 않게 약간의 딜레이
                        showNotification(`👻 유령 사탕 ${candyGained}개 획득!`, '#a29bfe');
                    }, 1500);
                }
            }
            ...
            let neonProb = MUTATION_CONFIG.NEON.probability;
            let danyuProb = MUTATION_CONFIG.DANYU.probability;
            let skyBlueProb = MUTATION_CONFIG.SKY_BLUE.probability;
            if (stats.customMutationProbabilities && typeof stats.customMutationProbabilities === 'object') {
                ...
                neonProb = stats.customMutationProbabilities.neon ?? neonProb;
                danyuProb = stats.customMutationProbabilities.danyu ?? danyuProb;
                skyBlueProb = stats.customMutationProbabilities.sky_blue ?? skyBlueProb;
            }

            // 변이(Mutation) 확률 계산
            let mutation = []; // 여러 변이를 담기 위해 배열로 변경

            // 할로윈 이벤트: 유령이 깃든 변이 우선 적용
            const isHalloween = isHalloweenPeriod();
            let hauntedProb = isHalloween ? MUTATION_CONFIG.HAUNTED.probability : 0;

            if (isHalloween && Math.random() * 100 < hauntedProb) {
                mutation.push('haunted');
            } else {
                // 유령이 깃들지 않았을 때만 다른 변이 계산
                ...
                if (window.isClubEventActive && (Math.random() * 100 < neonProb)) mutation.push('neon');
                if (typeof isStratosphereEventActive === 'function' && isStratosphereEventActive() && (Math.random() * 100 < skyBlueProb)) mutation.push('sky_blue');
                
                if (window.isDanYuEventForced && (Math.random() * 100 < danyuProb)) {
                    mutation.push('danyu');
                    if (typeof showDanYuEffect === 'function') {
                        showDanYuEffect();
                    }
                }
            }
            ...
            setTimeout(() => { showGachaChoice({ gradeKey: resultGradeKey, imagePath, itemName, grade, finalCoins, baseCoins, mutation }); updateCommonStats({ gradeKey: resultGradeKey, imagePath, mutation }); }, 1200);
