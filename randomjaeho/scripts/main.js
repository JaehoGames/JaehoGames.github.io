...
function showGachaChoice(result) {
    ...
    if (Array.isArray(mutation)) {
        ...
        // 할로윈 힌트 추가
        if (mutation.includes('haunted')) {
            choiceMutationText.innerHTML += `<br><small style="color: #a29bfe; opacity: 0.8;">(이 재호는 어딘가... 서늘한 기운이 느껴진다.)</small>`;
        }
...
/**
 * 유령 사탕 개수 UI를 업데이트합니다.
 */
function updateGhostCandyDisplay() {
    const candyAmountEl = document.getElementById('ghostCandyAmount');
    if (candyAmountEl) {
        candyAmountEl.textContent = (stats.ghostCandy || 0).toLocaleString();
    }
}

/**
 * 할로윈 이벤트를 초기화합니다. (1주차: 분위기 조성)
 */
function initHalloweenEvent() {
    // 할로윈 이벤트 기간인지 확인 (지금은 항상 true로 설정하여 테스트)
    ...
    console.log("🎃 Happy Halloween! Initializing Halloween event...");

    // 1. 할로윈 배경음악으로 변경
    const bgmPlayer = document.getElementById('bgmPlayer');
    if (bgmPlayer) {
        // 개발자 참고: 'assets/audio/bgm_halloween.mp3' 파일을 추가해야 합니다.
        // 파일이 존재하지 않으면 기존 BGM이 계속 재생됩니다.
        const halloweenBgmSrc = 'assets/audio/bgm_halloween.mp3';
        
        // 파일 존재 여부를 확인하는 간단한 방법 (완벽하지 않음)
        const http = new XMLHttpRequest();
        http.open('HEAD', halloweenBgmSrc, false);
        http.send();
        
        if (http.status !== 404) {
            bgmPlayer.src = halloweenBgmSrc;
            // 설정에 따라 음악 재생
            if (stats.settings.music) {
                fadeAudio(bgmPlayer, 'in', 1500);
            }
        } else {
            console.warn('할로윈 BGM 파일을 찾을 수 없습니다. 기존 BGM을 유지합니다.');
        }
    }

    // 2. 으스스한 메시지 표시
    const spookyOverlay = document.getElementById('spookyMessageOverlay');
    if (spookyOverlay) {
        spookyOverlay.textContent = '마을에 으스스한 기운이 감돌며 \'유령 사탕\'이 나타나기 시작했습니다... 사탕과 유령 재호는 다음 주에 큰 역할을 할지도 모릅니다.';
        
        // 1.5초 후 메시지 보이기
        setTimeout(() => {
            spookyOverlay.style.opacity = '1';
        }, 1500);

        // 6초 후 메시지 숨기기 (사라지는 시간 포함)
        setTimeout(() => {
            spookyOverlay.style.opacity = '0';
        }, 6000);
    }

    // 3. 거미줄 장식 추가
    const decorationsContainer = document.getElementById('halloween-decorations');
    if (decorationsContainer) {
        // 기존 장식 제거 (중복 방지)
        decorationsContainer.innerHTML = '';

        const deco1 = document.createElement('div');
        deco1.className = 'halloween-decoration cobweb-top-left';
        deco1.textContent = '🕸️';
        
        const deco2 = document.createElement('div');
        deco2.className = 'halloween-decoration cobweb-top-right';
        deco2.textContent = '🕸️';

        decorationsContainer.appendChild(deco1);
        decorationsContainer.appendChild(deco2);
    }

    // 4. 유령 사탕 UI 표시 및 초기화
    const candyDisplay = document.getElementById('ghostCandyDisplay');
    if (candyDisplay) {
        candyDisplay.style.display = 'flex';
    }
    updateGhostCandyDisplay();
}
...
function initChatSystem() {
    ...
    let chatMessageTimestamps = []; // 메시지 타임스탬프를 저장할 배열 초기화
    // 메시지 전송 기능
...
/**
 * 성층권 이벤트가 활성화 상태인지 확인합니다.
 * @returns {boolean}
 */
function isStratosphereEventActive() {
    // 개발자 강제 실행 확인
    if (window.isStratosphereEventForced) {
        return true;
    }
    ...
    // 매 5시간마다 정각부터 10분간
    return hours % 5 === 0 && minutes < 10;
}

function updateStratosphereEventUI() {
    const stratosphereCountdownContainer = document.getElementById('stratosphereEventCountdown');
    const stratosphereCountdownTextEl = document.getElementById('stratosphereCountdownText');
    const stratosphereCountdownTimerEl = document.getElementById('stratosphereCountdownTimer');

    if (!stratosphereCountdownContainer || !stratosphereCountdownTextEl || !stratosphereCountdownTimerEl) {
        return;
    }
    ...
    const isStratosphereActive = isStratosphereEventActive();
    let timeLeft, endTime;

    document.body.classList.toggle('stratosphere-event-active', isStratosphereActive);

    if (isStratosphereActive) {
        stratosphereCountdownContainer.style.display = 'flex';
        
        // 강제 실행 시 종료 시간
        if (window.isStratosphereEventForced) {
            endTime = window.stratosphereEventForcedEndTime ? window.stratosphereEventForcedEndTime.toDate() : new Date(now.getTime() + 10 * 60 * 1000);
        } else {
            const currentHour = kstNow.getHours();
            const nextEventEndMinute = 10;
            endTime = new Date(kstNow.getFullYear(), kstNow.getMonth(), kstNow.getDate(), currentHour, nextEventEndMinute, 0);
        }
        ...
        if (timeLeft <= 0) {
            stratosphereCountdownTextEl.textContent = '성층권 이벤트';
            stratosphereCountdownTimerEl.textContent = '종료!';
            return;
        }

        stratosphereCountdownTextEl.textContent = '🌌 성층권 이벤트 종료까지';
        ...
        if (nextEventHour >= 24) {
            endTime.setDate(endTime.getDate() + 1);
            endTime.setHours(nextEventHour % 24, 0, 0, 0);
        } else {
            endTime.setHours(nextEventHour, 0, 0, 0);
        }
        
        timeLeft = endTime.getTime() - now.getTime();
        stratosphereCountdownTextEl.textContent = '다음 성층권 이벤트까지';
    }
    ...
    stratosphereCountdownTimerEl.textContent =
        `${String(hours).padStart(2, '0')}:` +
        `${String(minutes).padStart(2, '0')}:` +
        `${String(seconds).padStart(2, '0')}`;
}
...
/** 성층권 이벤트 시스템 초기화 */
function initStratosphereEventSystem() {
    setInterval(updateStratosphereEventUI, 1000);
    updateStratosphereEventUI();
}
/**
