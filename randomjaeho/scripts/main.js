...
function updateCommonStats(result) {
    ...
    if (activeEffects.guaranteeEpic > 0) {
        activeEffects.guaranteeEpic--;
        // 만약 레어 보장권과 중복 사용되었다면, 더 높은 등급 보장권이므로 레어 보장권도 함께 차감
        if (activeEffects.guaranteeRare > 0) {
            activeEffects.guaranteeRare = 0;
        }
    }

    if (activeEffects.guaranteeLegendary > 0) {
        activeEffects.guaranteeLegendary--;
        if (activeEffects.guaranteeEpic > 0) activeEffects.guaranteeEpic = 0;
        if (activeEffects.guaranteeRare > 0) activeEffects.guaranteeRare = 0;
    }
...
function updateInventoryDisplay() {
    ...
    // updateExpandInventoryButton(); // 인벤토리 확장 기능 제거
}
...
function initShopSystem() {
    ...
    setInterval(() => {
        if (Object.values(activeEffects).some(value => value > 0)) {
            ...
            markDataDirty(); // 변경사항이 있을 수 있으므로 저장 대상으로 표시
        }
    }, 5000);
}
        ...
        try {
            ...
            if (saveNotificationToggle) {
                saveNotificationToggle.addEventListener('change', (e) => {
                    ...
                    markDataDirty(); // 설정을 변경했으므로 저장 대상으로 표시
                    if (stats.settings.showSaveNotifications) {
                        showNotification('저장 알림이 켜졌습니다.', '#2ecc71');
                    } else {
                        showNotification('저장 알림이 꺼졌습니다.', '#e74c3c');
                    }
                });
            }
            ...
            // 10초마다 데이터 저장
            setInterval(() => {
                if (isDataDirty && currentUser) {
                    isDataDirty = false; // 저장 시작 전에 플래그를 초기화
                    saveGameData();
                }
            }, 10000);
