...
function manualSave() {
    ...
    if (manualSaveTimestamps.length >= 10) {
...
function showNotification(message, color = '#2ecc71') {
    ...
    // 나타나는 애니메이션을 위해 'show' 클래스를 추가합니다. 짧은 지연을 주어 브라우저가 transition을 인지하도록 합니다.
    setTimeout(() => {
        toast.classList.add('show');
    }, 10); // 10ms 지연

    // 3초 후에 알림을 제거하는 로직을 설정합니다.
    setTimeout(() => {
        // 사라지는 애니메이션을 위해 'hide' 클래스를 추가합니다.
        toast.classList.remove('show');
        toast.classList.add('hide');

        // 애니메이션이 끝난 후 DOM에서 요소를 완전히 제거합니다.
        // CSS의 transition 시간(0.5s)과 일치시킵니다.
...
/**
 * 관리자/개발자용 특별 알림을 화면 중앙에 표시합니다.
 * @param {string} title - 알림 제목
 * @param {string} message - 알림 메시지
 * @param {string} icon - 표시할 아이콘 (이모지)
 * @param {'info' | 'success' | 'warning' | 'error'} type - 알림 종류 (테두리 색상 결정)
 */
function showAdminNotification(title, message, icon, type = 'info') {
...
/**
 * Firestore Timestamp를 "N분 전"과 같은 상대 시간으로 변환합니다. (dev.js용)
...
/**
 * Firestore Timestamp를 "일/오전/오후 HH:MM" 형식으로 변환합니다.
 * @param {firebase.firestore.Timestamp} timestamp - Firestore 타임스탬프 객체
 * @returns {string} 변환된 시간 문자열
 */
function formatChatTimestamp(timestamp) {
    ...
    return `${day}일/${ampm} ${formattedHours}:${formattedMinutes}`;
}
...
async function saveGameData(forceNotification = false) {
    if (currentUser) {
        ...
        // 비로그인 상태: 아무것도 저장하지 않음. main.js에서 알림을 처리합니다.
