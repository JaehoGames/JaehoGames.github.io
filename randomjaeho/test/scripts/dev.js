...
async function updateUserMutationProbabilities() {
    ...
    try {
        ...
        if (isNaN(goldProb) || isNaN(rainbowProb) || isNaN(flameProb) || isNaN(danyuProb) || isNaN(neonProb) || goldProb < 0 || rainbowProb < 0 || flameProb < 0 || danyuProb < 0 || neonProb < 0) {
            throw new Error('유효하지 않은 확률 값입니다. 0 이상의 숫자를 입력하세요.');
        }

        const newMutationProbs = { gold: goldProb, rainbow: rainbowProb, flame: flameProb, danyu: danyuProb, neon: neonProb };

        if (uid === 'all') {
            // 모든 유저에게 변이 확률 적용
            if (!confirm(`정말로 모든 유저의 변이 확률을 골드 ${goldProb}%, 레인보우 ${rainbowProb}%, 화염 ${flameProb}%, 최단유 ${danyuProb}%, 네온 ${neonProb}%로 설정하시겠습니까?`)) {
                return;
            }
